...
<head>
    ...
    <title>@yield('title', 'Portal Orang Tua | RA AN-NUUR')</title>
    ...
    <style>
        ...
        .sidebar-active::after { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 4px; background-color: #6d5e00; }
...
<body class="bg-background text-on-surface">
    ...
    <header class="md:ml-[280px] sticky top-0 bg-white/80 backdrop-blur-md px-4 md:px-8 py-3 flex justify-between md:justify-end items-center z-40 border-b border-gray-100">
        <div class="flex items-center gap-3 md:hidden">
            ...
            <h1 class="font-bold text-primary text-lg">RA AN-NUUR</h1>
        </div>
        
        <div class="flex items-center gap-4">
            <!-- Notification Bell -->
            @php 
                $activeTicketsCount = \App\Models\Ticket::where('reporter_id', Auth::id())
                    ->whereIn('status', ['open', 'in_progress'])
                    ->count();
            @endphp
            <a href="{{ route('parent.helpdesk.index') }}" class="relative p-2 text-gray-400 hover:text-emerald-500 transition-colors hover:bg-emerald-50 rounded-full flex items-center justify-center" title="Notifikasi Bantuan">
    ...
    <!-- Footer -->
    <footer class="md:ml-[280px] bg-surface-container-lowest border-t border-outline-variant">
    ...
    <div x-data="{ mobileMoreOpen: false }">
        <nav class="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 shadow-[0_-4px_24px_rgba(0,0,0,0.05)] z-50 px-2 pb-2 pt-2 flex justify-around items-center">
            ...
            <a href="{{ route('parent.dashboard') }}" class="flex flex-col items-center p-2 {{ $currentRoute === 'parent.dashboard' ? 'text-emerald-500' : 'text-gray-400 hover:text-emerald-500' }}">
                <span class="material-symbols-outlined {{ $currentRoute === 'parent.dashboard' ? 'font-bold' : '' }}" style="{{ $currentRoute === 'parent.dashboard' ? 'font-variation-settings: \'FILL\' 1;' : '' }}">dashboard</span>
                <span class="text-[10px] font-medium mt-1">Beranda</span>
            </a>
            
            <a href="{{ route('parent.status') }}" class="flex flex-col items-center p-2 {{ $currentRoute === 'parent.status' ? 'text-emerald-500' : 'text-gray-400 hover:text-emerald-500' }}">
                <span class="material-symbols-outlined {{ $currentRoute === 'parent.status' ? 'font-bold' : '' }}" style="{{ $currentRoute === 'parent.status' ? 'font-variation-settings: \'FILL\' 1;' : '' }}">assignment</span>
                <span class="text-[10px] font-medium mt-1">Status</span>
            </a>

            <a href="{{ route('parent.documents') }}" class="flex flex-col items-center p-2 {{ $currentRoute === 'parent.documents' ? 'text-emerald-500' : 'text-gray-400 hover:text-emerald-500' }}">
                <span class="material-symbols-outlined {{ $currentRoute === 'parent.documents' ? 'font-bold' : '' }}" style="{{ $currentRoute === 'parent.documents' ? 'font-variation-settings: \'FILL\' 1;' : '' }}">folder_open</span>
                <span class="text-[10px] font-medium mt-1">Dokumen</span>
            </a>

            <a href="{{ route('parent.announcements') }}" class="flex flex-col items-center p-2 {{ $currentRoute === 'parent.announcements' ? 'text-emerald-500' : 'text-gray-400 hover:text-emerald-500' }}">
                <span class="material-symbols-outlined {{ $currentRoute === 'parent.announcements' ? 'font-bold' : '' }}" style="{{ $currentRoute === 'parent.announcements' ? 'font-variation-settings: \'FILL\' 1;' : '' }}">notifications</span>
                <span class="text-[10px] font-medium mt-1">Info</span>
            </a>

            <button @click="mobileMoreOpen = true" class="flex flex-col items-center p-2 {{ in_array($currentRoute, ['parent.profile', 'parent.student.edit']) ? 'text-emerald-500' : 'text-gray-400 hover:text-emerald-500' }}">
                <span class="material-symbols-outlined {{ in_array($currentRoute, ['parent.profile', 'parent.student.edit']) ? 'font-bold' : '' }}">menu</span>
        ...
        <div x-show="mobileMoreOpen" style="display: none;" class="md:hidden fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
            ...
            <div x-show="mobileMoreOpen" 
                ...
                <div class="grid grid-cols-2 gap-4">
                    <a href="{{ route('parent.profile') }}" class="flex flex-col items-center p-4 bg-gray-50 rounded-2xl hover:bg-emerald-50 hover:text-emerald-600 transition-colors">
                        <span class="material-symbols-outlined mb-2 text-emerald-500">settings</span>
                        <span class="text-xs font-bold text-gray-700">Pengaturan Profil</span>
                    </a>
                    
                    <a href="{{ route('parent.helpdesk.index') }}" class="flex flex-col items-center p-4 bg-gray-50 rounded-2xl hover:bg-emerald-50 hover:text-emerald-600 transition-colors">
                        <span class="material-symbols-outlined mb-2 text-emerald-500">support_agent</span>
                        <span class="text-xs font-bold text-gray-700">Pusat Bantuan</span>
                    </a>
                </div>
