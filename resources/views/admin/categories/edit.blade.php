...
@section('title', 'Edit Kategori | Admin RA AN-NUUR')
...
<main class="md:ml-[280px] min-h-screen px-4 md:px-8 py-8 bg-gray-50">
    <section class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            ...
            <h2 class="font-display text-2xl md:text-3xl font-bold text-gray-900">Edit Kategori</h2>
        </div>
    </section>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8 max-w-2xl">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="name" value="{{ old('name', $category->name) }}" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all outline-none" placeholder="Masukkan nama kategori">
                @error('name')
            ...
            <div class="flex items-center gap-3 mt-8">
                ...
                <button type="submit" class="px-5 py-2.5 rounded-xl bg-emerald-500 text-white font-bold hover:bg-emerald-600 transition-colors shadow-md shadow-emerald-500/20">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</main>
