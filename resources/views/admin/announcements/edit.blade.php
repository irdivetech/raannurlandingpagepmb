...
@section('title', 'Edit Pengumuman | Admin RA AN-NUUR')
...
<main class="md:ml-[280px] min-h-screen px-4 md:px-8 py-8 bg-gray-50">
    ...
    <section class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
            ...
            <h2 class="font-display text-3xl md:text-4xl font-bold text-gray-900">Edit Pengumuman</h2>
            <p class="text-gray-500 mt-1">Perbarui informasi pengumuman yang sudah ada</p>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-gray-100 flex items-center gap-3 bg-gray-50/50">
                <div class="w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center">
                    <span class="material-symbols-outlined text-amber-500 text-[20px]">edit</span>
                </div>
                <div>
                    <h3 class="font-bold text-gray-900 text-lg">Edit Pengumuman</h3>
                    <p class="text-xs text-gray-500 mt-0.5">Perubahan akan langsung terlihat oleh orang tua jika status Aktif</p>
                </div>
            </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="content" name="content" rows="8" required
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-800 text-sm focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20 focus:bg-white transition-all resize-y">{{ old('content', $announcement->content) }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-xs mt-1.5 flex items-center gap-1"><span class="material-symbols-outlined text-[13px]">error</span> {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-0.5">Jika tidak dicentang, pengumuman disembunyikan dari portal orang tua.</p>
                        </div>
                    </div>
                </div>
